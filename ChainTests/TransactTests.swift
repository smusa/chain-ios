...
class TransactTests : BaseTests {
    ...
    func testTransact() {
        ...
        shouldCompleteIn(2000.0)

        self.client.transact([
            "inputs": [
                ["private_key": key1.WIF],
                ["private_key": key2.WIF],
                ["private_key": key3.WIF],
            ],
            "outputs": [
                [ "address": "mrn4jgT5gGLZjK36CAGWZj917meq9Rk3Gv", "amount": 13500000 ],
                [ "address": "mzRf7M6cUzRWWPCUPNu5f9swpAaBZBKg8K", "amount": 1200000 ],
                ...
                NSLog("BROADCASTED TRANSACTION: %@", dictionary)
                let txid = BTCDataWithHexString(dictionary["transaction_hash"] as String)

                XCTAssert(txid.length == 32, "Must return a 256-bit transaction ID in hex.")
